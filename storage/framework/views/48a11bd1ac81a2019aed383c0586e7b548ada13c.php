

<title>FAQs</title>
<?php $__env->startSection('content'); ?>
<div class="card mb-5 mb-xl-8">
    <div class="card-body py-lg-8 py-sm-0 mb-4">

        <!--begin::Sign-in Method-->
        <div class="card mb-5 mb-xl-7">
            <!--begin::Card header-->
            <div class="card-header border-0 cursor-pointer" role="button">
                <div class="card-title m-0">
                    <h2 class="fw-bold m-0 ">Welcome to PipCollective 🎉</h2>
                </div>
            </div>
            <!--end::Card header-->
            <!--begin::Card body-->
            <div class="card-body border-top">
                <iframe src="https://player.vimeo.com/video/456269546?h=0ed43adde6&color=ffffff&title=0&byline=0&portrait=0" class="vimeo_video" frameborder="0" allow="autoplay; fullscreen; picture-in-picture" allowfullscreen></iframe>
                <div class="row mt-10 mb-10">
                    <div class="col-md-6 col-12 mb-5">
                        <div class="card">
                            <div class="card-body">
                                <div class="d-flex flex-column">
                                    <div class="d-md-flex flex-row align-items-center gap-5">
                                        <div>
                                            <i class="fa-brands fa-discord fs-2hx"></i>
                                        </div>
                                        <div class="d-flex flex-column">
                                            <div>
                                                <h3>Join our Discord</h3>
                                            </div>
                                            <div>
                                                <p>Full of like minded traders, all on the journey to success</p>
                                            </div>
                                        </div>
                                    </div>
                                    <div>
                                        <button class="btn btn-primary btn-block w-100">Join Our Discord</button>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>


                    <div class="col-md-6 col-12 mb-5">
                        <div class="card">
                            <div class="card-body">
                                <div class="d-flex flex-column">
                                    <div class="d-md-flex flex-row align-items-center gap-5">
                                        <div>
                                            <i class="ki-duotone ki-question fs-2hx">
                                             <span class="path1"></span>
                                             <span class="path2"></span>
                                            </i>
                                        </div>
                                        <div class="d-flex flex-column">
                                            <div>
                                                <h3>Need Help</h3>
                                            </div>
                                            <div>
                                                <p>Create a ticket now to get the support you need from our team?</p>
                                            </div>
                                        </div>
                                    </div>
                                    <div>
                                        <button class="btn btn-primary btn-block w-100">Create a Support ticket</button>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>


                    <div class="col-md-6 col-12 mb-5">
                        <div class="card">
                            <div class="card-body">
                                <div class="d-flex flex-column">
                                    <div class="d-md-flex flex-row align-items-center gap-5">
                                        <div>
                                            <i class="ki-duotone ki-cloud-download fs-2hx">
                                             <span class="path1"></span>
                                             <span class="path2"></span>
                                            </i>
                                        </div>
                                        <div class="d-flex flex-column">
                                            <div>
                                                <h3>Download</h3>
                                            </div>
                                            <div>
                                                <p>Get started now with The PipCollective organization Pack</p>
                                            </div>
                                        </div>
                                    </div>
                                    <div>
                                        <button class="btn btn-primary btn-block w-100">Download</button>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>


                    <div class="col-md-6 col-12 mb-5">
                        <div class="card">
                            <div class="card-body">
                                <div class="d-flex flex-column gap-5">
                                    <div>
                                        <h3>Get started now with ThePipCollective organization Pack</h3>
                                    </div>
                                    <div>
                                        <button class="btn btn-primary btn-block w-100">Download Here</button>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>


                    <div class="col-md-6 col-12 mb-5">
                        <div class="card">
                            <div class="card-body">
                                <div class="d-flex flex-column gap-5">
                                    <div>
                                        <h3>Checkout our self learning AI support bot, knowledgeable on all things we do here at ThePipCollective.</h3>
                                    </div>
                                    <div>
                                        <h4 class="text-primary">Click the chat icon on the bottom right of this page or our website.</h4>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>

                </div>
            </div>
            <!--end::Card body-->
        </div>
        <!--end::Sign-in Method-->
    </div>
    <!--end::Body-->
</div>
<?php $__env->stopSection(); ?>
<?php echo $__env->make('componant.main-layout', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\xampp\htdocs\pipcollective\resources\views/users/dashboard.blade.php ENDPATH**/ ?>